...
POSTGIS_TABLE = dict(
    ...
    password='12345',
    dbname='kelasgis',
    table='(select ST_MPolyFromText(ST_AsText(geom)) as geom, nama from bawean) as bawean')
LAYER_NAME = 'pantai'
...
def render_tile(layer, z, x, y):
    ...
    line_symbolizer.fill_opacity = 0.5
    #line_symbolizer.allow_overlap = True
    #line_symbolizer.transform="scale(5,5)"
    r.symbols.append(line_symbolizer)

    label = mapnik.TextSymbolizer(mapnik.Expression('[nama]'), 'DejaVu Sans Book',24,mapnik.Color('blue'
    ))
    label.halo_radius = 1
